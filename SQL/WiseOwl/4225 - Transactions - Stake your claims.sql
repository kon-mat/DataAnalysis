...
WHERE 
	CountryName LIKE '%United kingdom%' --Add 'My country' to which ever country you call home
...
WHERE 
	CountryName NOT LIKE '%United kingdom%' --Start by adding Holiday to all countries but your 
...
IF 1 + 1 = 2 --Check if this impossible question is true
	BEGIN
		...
		FROM 
			tblCountry 
		ORDER BY 
			CountryName	-- Prove the update didn't happen	
